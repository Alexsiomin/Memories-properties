...
const FALLBACK_SELECTIVITY: InsightRow[] = [
  { id: '1', section: 'selectivity_limassol', label: 'Germasogeia', value: 'Limassol', sub: 'Seafront villa', numeric_value: 12.4, numeric_x: null, numeric_y: null, category: 'Q1 2026', sort_order: 1, is_active: true },
  { id: '2', section: 'selectivity_limassol', label: 'Agios Tychonas', value: 'Limassol', sub: 'Beachfront apartment', numeric_value: 3.8, numeric_x: null, numeric_y: null, category: 'Q1 2026', sort_order: 2, is_active: true },
  { id: '3', section: 'selectivity_limassol', label: 'Amathus', value: 'Limassol', sub: 'Resort residence', numeric_value: 18.5, numeric_x: null, numeric_y: null, category: 'Q4 2025', sort_order: 3, is_active: true },
];
...
const InsightsLimassol = () => {
  const { data: selectivity } = useInsights('selectivity_limassol', FALLBACK_SELECTIVITY);
  const { data: assetMix } = useInsights('asset_mix_limassol');
  const { data: priceTrend } = useInsights('price_trend_limassol');
  const { data: deals } = useInsights('deals_limassol');
  const { data: topApartments } = useInsights('top_apartments_limassol');
  const { data: topPlots } = useInsights('top_plots_limassol');
  const { data: topNewDevelopments } = useInsights('top_new_developments_limassol');

  return (
    <>
      <SEO
        title="Limassol Property Market Insights & Sold Prices | Memories"
        description="Track Limassol real estate trends, recent sales, asset mix and price benchmarks with Memories' market insights."
        type="article"
        jsonLd={{
          '@context': 'https://schema.org',
          '@type': 'Article',
          headline: 'Limassol Property Market Insights & Sold Prices',
          description:
      ...
      >
        <PageHeader
          eyebrow="LIMASSOL PROPERTY MARKET INSIGHTS"
        ...
        <SalesTable title="MOST EXPENSIVE PLOTS SOLD IN LIMASSOL" rows={topPlots} />
        <SalesTable title="Top Contact of Sale Transactions In Limassol" rows={topNewDevelopments} />
        ...
        <div className="container mx-auto px-6 max-w-6xl py-16 flex flex-col items-center gap-4">
          ...
          >
            <span className="story-link whitespace-nowrap">Paphos Market Insights</span> <span aria-hidden>→</span>
          </Link>
          <Link
            to="/contact"
            className="btn-cta btn-cta-solid group px-4 sm:px-8"
...
export default InsightsLimassol;
