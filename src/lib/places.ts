...
type LocalPlace = {
  ...
  /** Parent city/town when the place is a neighborhood (e.g. Universal -> Paphos). */
  city?: string;
  aliases?: string[];
};
...
/** Format a Google place prediction to "Locality, District" (drop the country suffix). */
export function formatPlace(text: string): string {
  const parts = text.split(',').map((p) => p.trim()).filter(Boolean);
  const local = findLocalPlace(parts);
  if (local) return display(local);

  // Drop a trailing country (e.g. "Cyprus") so we keep "Kato Paphos, Paphos".
  if (parts.length > 1 && /cyprus/i.test(parts[parts.length - 1])) parts.pop();
  return parts.slice(0, 2).join(', ');
}

function isAllowedSuggestion(raw: string): boolean {
...
/**
 * Fetch Cyprus (Paphos/Limassol) location suggestions for the given input.
 *
 * Routed through the `places-autocomplete` edge function (connector gateway)
 * rather than the browser Places JS SDK. The browser key is referrer-locked
 * to specific domains, which breaks autocomplete in the editor preview and on
 * localhost. The gateway runs server-side with no referrer restrictions, so it
 * works consistently across every environment.
 */
export async function fetchLocationSuggestions(input: string): Promise<string[]> {
  const q = input.trim();
  if (q.length < 2) return [];
  ...
  try {
    ...
    // Keep autocomplete usable from the curated Paphos/Limassol list even if
    // the remote Places request is temporarily unavailable.
  }
